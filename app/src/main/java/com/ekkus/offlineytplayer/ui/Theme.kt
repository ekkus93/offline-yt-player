...
internal object ThemeContrastPolicy {
    ...
    fun ratio(foreground: Color, background: Color): Double { val lighter = maxOf(luminance(foreground), luminance(background)); val darker = minOf(luminance(foreground), luminance(background)); return (lighter + 0.05) / (darker + 0.05) }
    private fun luminance(color: Color): Double { fun channel(value: Float): Double { val c = value.toDouble(); return if (c <= 0.04045) c / 12.92 else ((c + 0.055) / 1.055).pow(2.4) }; return 0.2126 * channel(color.red) + 0.7152 * channel(color.green) + 0.0722 * channel(color.blue) }
}

@Composable
fun OfflineYTPlayerTheme(preference: AppearanceSetting = AppearanceSetting.System, content: @Composable () -> Unit) {
    ...
    MaterialTheme(colorScheme = if (useDark) DarkColors else LightColors, typography = MidnightTransitTypography, content = content)
}
