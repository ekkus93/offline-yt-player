...
@Composable internal fun LibraryScreen(onAdd:()->Unit,onPlay:(LocalPlaybackAsset)->Unit,state:LibraryScreenState=LibraryScreenState.Unavailable("Library repository is not connected yet; no empty-library claim is being made."),settings:AppSettingsSnapshot=AppSettingsSnapshot(),onUpdateSettings:(AppSettingsMutation.()->Unit)->Unit={}) {
 ...
 Column(Modifier.fillMaxSize().padding(MidnightTransit.ScreenSpacing),verticalArrangement=Arrangement.spacedBy(MidnightTransit.SectionSpacing)){
  ...
  Text("Filter: All · ${layout.name}"); message?.let{Text(it)}
  when(state){ LibraryScreenState.Loading->RepositoryStatus("Loading library repository state…"); is LibraryScreenState.Unavailable->RepositoryStatus(state.reason); is LibraryScreenState.Failed->RepositoryStatus("Library repository failed: ${state.message}"); is LibraryScreenState.Ready->{ if(visibleItems.isEmpty()){Column(Modifier.weight(1f).fillMaxWidth(),verticalArrangement=Arrangement.Center,horizontalAlignment=Alignment.CenterHorizontally){Text(if(query.isBlank())"No offline videos yet" else "No matching videos")};Button(onClick=onAdd,modifier=Modifier.fillMaxWidth().sizeIn(minHeight=MidnightTransit.MinimumTouchTarget)){Text("Add video")}}else LibraryItems(layout,visibleItems,onPlay,{message="${it.title}: ${it.detail}"},{message="Remove requires repository-backed deletion for ${it.title}."}) } }
  if(state !is LibraryScreenState.Ready)Button(onClick=onAdd,modifier=Modifier.fillMaxWidth().sizeIn(minHeight=MidnightTransit.MinimumTouchTarget)){Text("Add video")}
 }
}
...
private fun LibraryRowModel.matchesLibraryQuery(query:String):Boolean{val n=query.trim();return n.isBlank()||title.contains(n,true)||detail.contains(n,true)}
