...
- The durable-state polling budget is 25 ms. `DownloadEngine` additionally checks its stop flag before each 64 KiB response-body read, so once the execution loop polls durable state, pause observation is bounded by the poll interval plus the currently blocked network read/request timeout.
...
The remaining RMD-503 execution-loop step is to poll this bridge while each transfer is in flight and classify a cooperative stop as Pause versus terminal Cancel from durable state. Resume remains separately qualified under RMD-504.
