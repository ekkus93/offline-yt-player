...
pub struct FfiDownloadControlService { library: LibraryStore }
...
impl FfiDownloadControlService {
    ...
    pub fn open(database_path: String) -> Result<Arc<Self>, FfiDownloadControlOpenError> {
        LibraryStore::open(&database_path)
            ...
            .map_err(|error| FfiDownloadControlOpenError::Persistence { message: error.message })
    }
    ...
    /// Resolving -> Downloading transition so a resumed item cannot masquerade as active before it
    /// has actually been claimed. DownloadEngine then revalidates any retained partial before use.
...
impl FfiDownloadControlService {
    fn enqueue_inner(&self, job_id: &str) -> Result<bool, CoreError> {
        if job_id.trim().is_empty() {
            return Err(CoreError::new(ErrorKind::InvalidInput, "download job id must not be empty", false));
        }
        ...
        self.library.save_download_snapshot(&DurableDownloadSnapshot {
            ...
            total_bytes: None, attempt: 0, retry_at_epoch_ms: None, last_error: None,
        })?;
        Ok(true)
    }
    ...
    fn transition_inner(&self, job_id: &str, next: DownloadState) -> Result<bool, CoreError> {
        ...
        let snapshot = snapshots.iter_mut().find(|snapshot| snapshot.job_id == job_id).ok_or_else(|| {
            CoreError::new(ErrorKind::InvalidInput, "download job does not exist", false)
        })?;
...
mod tests {
    ...
    fn snapshot(job_id: &str, state: DownloadState) -> DurableDownloadSnapshot {
        DurableDownloadSnapshot { job_id: job_id.into(), state, bytes_downloaded: 128,
            total_bytes: Some(1024), attempt: 0, retry_at_epoch_ms: None, last_error: None }
    }
    ...
    fn enqueue_persists_a_new_queued_job_and_is_idempotent() {
        ...
        assert!(first.updated); assert!(first.error.is_none());
        let saved = store.load_download_snapshots().unwrap();
        assert_eq!(saved.len(), 1); assert_eq!(saved[0].state, DownloadState::Queued);
        let repeated = service.enqueue("job-new".into());
        assert!(!repeated.updated); assert!(repeated.error.is_none());
    }
    ...
    fn enqueue_rejects_empty_job_id_with_typed_error() {
        ...
        assert!(!result.updated); assert_eq!(result.error.unwrap().kind, FfiErrorKind::InvalidInput);
    }
    ...
    fn pause_resume_and_cancel_persist_legal_transitions() {
        ...
        let database = temp.path().join("library.sqlite3");
        let store = LibraryStore::open(&database).unwrap();
        store.save_download_snapshot(&snapshot("job-1", DownloadState::Downloading)).unwrap();
        let service = FfiDownloadControlService::open(database.to_string_lossy().into_owned()).unwrap();
        assert!(service.pause("job-1".into()).updated);
        assert_eq!(store.load_download_snapshots().unwrap()[0].state, DownloadState::Paused);
        assert!(service.resume("job-1".into()).updated);
        assert_eq!(store.load_download_snapshots().unwrap()[0].state, DownloadState::Queued);
        assert!(service.cancel("job-1".into()).updated);
        assert_eq!(store.load_download_snapshots().unwrap()[0].state, DownloadState::Canceled);
    }

    #[test]
    fn resume_survives_reopen_and_remains_worker_eligible() {
        let temp = tempfile::tempdir().unwrap();
        let database = temp.path().join("library.sqlite3");
        let store = LibraryStore::open(&database).unwrap();
        store.save_download_snapshot(&snapshot("paused", DownloadState::Paused)).unwrap();
        drop(store);
        let service = FfiDownloadControlService::open(database.to_string_lossy().into_owned()).unwrap();
    ...
    fn invalid_transition_and_missing_job_are_typed_errors() {
        ...
        let database = temp.path().join("library.sqlite3");
        let store = LibraryStore::open(&database).unwrap();
        store.save_download_snapshot(&snapshot("done", DownloadState::Completed)).unwrap();
        let service = FfiDownloadControlService::open(database.to_string_lossy().into_owned()).unwrap();
    ...
    fn repeated_target_state_is_an_idempotent_no_op() {
        ...
        assert!(!result.updated); assert!(result.error.is_none());
    }
}
