        ...
        self.sources
            ...
            .ok_or_else(|| {
                CoreError::new(
                    ...
                    "This URL is not from a supported source",
                    false,
                )
            })
    }
...
pub struct DirectFixtureSource {
    entries: Arc<HashMap<String, FixtureMedia>>,
}
...
impl DirectFixtureSource {
    ...
    fn fixture(&self, url: &str) -> Result<&FixtureMedia, CoreError> {
        self.entries.get(url).ok_or_else(|| {
            CoreError::new(
                ErrorKind::UnsupportedSource,
                "Fixture URL is not registered",
                false,
            )
        })
    }
...
impl MediaSource for DirectFixtureSource {
        ...
        Box::pin(async move {
            ...
            Ok(MediaInfo {
                source: SourceIdentity {
                    ...
                    media_id: fixture.media_id.clone(),
                    canonical_url: Some(url.into()),
                },
                title: sanitize_title(&fixture.title),
                duration_ms: Some(fixture.duration_ms),
                thumbnail_url: fixture.thumbnail_url.clone(),
                formats: vec![MediaFormat {
                    format_id: "fixture-combined".into(),
                    container: "mp4".into(),
                    mime_type: Some("video/mp4".into()),
                    role: crate::domain::StreamRole::Combined,
                    bitrate_bps: None,
                    content_length: fixture.bytes,
                    video: Some(crate::domain::VideoFormat {
                        width: 1280,
                        height: 720,
                        fps: Some(30),
                        codec: "h264".into(),
                    }),
                    audio: Some(crate::domain::AudioFormat {
                        codec: "aac".into(),
                        bitrate_bps: None,
                        channels: Some(2),
                        language: None,
    ...
    fn choices<'a>(&'a self, media: &'a MediaInfo) -> SourceFuture<'a, Vec<QualityChoice>> {
        Box::pin(async move {
            let format = media.formats.first().ok_or_else(|| {
                CoreError::new(ErrorKind::NoCompatibleFormat, "No fixture format", false)
            })?;
            Ok(vec![QualityChoice {
                choice_id: "fixture-720p".into(),
                label: "720p".into(),
                estimated_bytes: format.content_length,
                video_height: Some(720),
                audio_only: false,
                compatibility: crate::domain::Compatibility::Preferred,
            }])
        })
    }
    ...
    fn download_plan<'a>(
        ...
        Box::pin(async move {
            if choice_id != "fixture-720p" {
                return Err(CoreError::new(
                    ErrorKind::NoCompatibleFormat,
                    "Unknown fixture quality",
                    false,
                ));
            }
            let canonical = media.source.canonical_url.as_deref().ok_or_else(|| {
                CoreError::new(ErrorKind::InvalidInput, "Missing canonical URL", false)
            })?;
...
mod tests {
    ...
    fn fixture_adapter_is_provider_independent() {
        let source = DirectFixtureSource::with_entries([(
            "https://fixture.invalid/watch/1".into(),
            FixtureMedia {
                media_id: "one".into(),
                title: "Example".into(),
                duration_ms: 10_000,
