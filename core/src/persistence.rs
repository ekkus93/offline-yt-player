...
impl LibraryStore {
    ...
    fn connection(&self) -> Result<MutexGuard<'_, Connection>, CoreError> {
        self.connection.lock().map_err(|_| {
            CoreError::new(
                ...
                "library database lock poisoned",
                false,
            )
        })
    }
    ...
    pub fn migrate(&self) -> Result<(), CoreError> {
        ...
        if version > SCHEMA_VERSION {
            return Err(CoreError::new(
                ErrorKind::Persistence,
                "library database is newer than this application",
                false,
            ));
        }
        if version < 1 {
            connection
                .execute_batch(
                    "BEGIN IMMEDIATE;
                     ...
                     CREATE UNIQUE INDEX library_source_identity
                       ON library_items(provider, source_media_id);
                     CREATE TABLE assets (
    ...
    pub fn promote_completed(&self, job_id: &str, item: &LibraryItem) -> Result<(), CoreError> {
        if !item.completed {
            return Err(CoreError::new(
                ...
                "cannot promote an incomplete library item",
                false,
            ));
        }
        ...
        transaction
            .execute("DELETE FROM staged_assets WHERE job_id=?1", [job_id])
            .map_err(db_error)?;
        ...
        let mut statement = connection
            .prepare(
                "SELECT item_id, provider, source_media_id, canonical_url, display_title,
                        duration_ms, quality_label, created_at_epoch_ms,
                        playback_position_ms, completed
                 FROM library_items
                 WHERE (?1 IS NULL OR display_title LIKE ?1 ESCAPE '\\')
                 ORDER BY created_at_epoch_ms DESC",
            )
            .map_err(db_error)?;
        let rows = statement
            .query_map([like.as_deref()], map_item_row)
            .map_err(db_error)?;
        let mut items = Vec::new();
        for row in rows {
            let mut item = row.map_err(db_error)?;
            item.assets = load_assets(&connection, &item.item_id)?;
            items.push(item);
        }
        Ok(items)
    }
    ...
    pub fn get(&self, item_id: &str) -> Result<Option<LibraryItem>, CoreError> {
        ...
        let mut item = statement
            .query_row([item_id], map_item_row)
            .optional()
            .map_err(db_error)?;
        if let Some(found) = &mut item {
            found.assets = load_assets(&connection, item_id)?;
        }
        Ok(item)
    }
    ...
    pub fn rename(&self, item_id: &str, title: &str) -> Result<bool, CoreError> {
        let changed = self
            .connection()?
            .execute(
                "UPDATE library_items SET display_title=?2 WHERE item_id=?1",
    ...
    pub fn save_playback_position(
        ...
        let changed = self
            .connection()?
            .execute(
                "UPDATE library_items SET playback_position_ms=?2 WHERE item_id=?1",
                params![item_id, to_i64(position_ms)?],
            )
            .map_err(db_error)?;
        Ok(changed == 1)
    }
    ...
    pub fn delete(&self, item_id: &str) -> Result<Option<LibraryItem>, CoreError> {
        ...
        if item.is_some() {
            self.connection()?
                .execute("DELETE FROM library_items WHERE item_id=?1", [item_id])
                .map_err(db_error)?;
        }
        Ok(item)
    }
    ...
    pub fn save_download_snapshot(
        &self,
        snapshot: &DurableDownloadSnapshot,
    ) -> Result<(), CoreError> {
            ...
            .execute(
                "INSERT INTO download_jobs(job_id, state_json, bytes_downloaded, total_bytes,
                 ...
                 ON CONFLICT(job_id) DO UPDATE SET
                   ...
                   retry_at_epoch_ms=excluded.retry_at_epoch_ms,
                   error_json=excluded.error_json",
                params![
                    snapshot.job_id,
                    state,
                    to_i64(snapshot.bytes_downloaded)?,
                    snapshot.total_bytes.map(to_i64).transpose()?,
                    i64::from(snapshot.attempt),
                    snapshot.retry_at_epoch_ms.map(to_i64).transpose()?,
                    error
                ],
            )
            .map_err(db_error)?;
    ...
    pub fn load_download_snapshots(&self) -> Result<Vec<DurableDownloadSnapshot>, CoreError> {
        ...
        let mut statement = connection
            .prepare(
                "SELECT job_id, state_json, bytes_downloaded, total_bytes, attempt,
                        retry_at_epoch_ms, error_json
                 FROM download_jobs ORDER BY job_id",
            )
            .map_err(db_error)?;
        let rows = statement
            .query_map([], |row| {
                ...
                Ok(DurableDownloadSnapshot {
                    job_id: row.get(0)?,
                    state,
                    bytes_downloaded: from_i64(row.get(2)?)?,
                    total_bytes: row.get::<_, Option<i64>>(3)?.map(from_i64).transpose()?,
                    attempt: u32::try_from(row.get::<_, i64>(4)?).map_err(|error| {
                        rusqlite::Error::FromSqlConversionFailure(
                            4,
    ...
    pub fn staged_job_ids(&self) -> Result<Vec<String>, CoreError> {
        let connection = self.connection()?;
        let mut statement = connection
            .prepare("SELECT DISTINCT job_id FROM staged_assets ORDER BY job_id")
            .map_err(db_error)?;
        let rows = statement
...
fn upsert_item(transaction: &Transaction<'_>, item: &LibraryItem) -> Result<(), CoreError> {
    transaction
        .execute(
            "INSERT INTO library_items(item_id, provider, source_media_id, canonical_url,
                 display_title, duration_ms, quality_label, created_at_epoch_ms,
                 playback_position_ms, completed)
            ...
            params![
                ...
                item.display_title,
                item.duration_ms.map(to_i64).transpose()?,
                item.quality_label,
                to_i64(item.created_at_epoch_ms)?,
                to_i64(item.playback_position_ms)?,
                i64::from(item.completed)
            ],
        )
        .map_err(db_error)?;
    Ok(())
...
fn insert_asset(
    ...
    transaction
        .execute(
            "INSERT INTO assets(item_id, asset_id, kind_json, relative_path, bytes, sha256, mime_type)
             VALUES(?1, ?2, ?3, ?4, ?5, ?6, ?7)",
            params![
                item_id,
                asset.asset_id,
                serde_json::to_string(&asset.kind).map_err(json_error)?,
                asset.relative_path,
                to_i64(asset.bytes)?,
                asset.sha256,
                asset.mime_type
...
fn map_item_row(row: &rusqlite::Row<'_>) -> rusqlite::Result<LibraryItem> {
    Ok(LibraryItem {
        ...
        duration_ms: row.get::<_, Option<i64>>(5)?.map(from_i64).transpose()?,
        quality_label: row.get(6)?,
        assets: Vec::new(),
        created_at_epoch_ms: from_i64(row.get(7)?)?,
        playback_position_ms: from_i64(row.get(8)?)?,
...
fn load_assets(connection: &Connection, item_id: &str) -> Result<Vec<LocalAsset>, CoreError> {
    let mut statement = connection
        .prepare(
            "SELECT asset_id, kind_json, relative_path, bytes, sha256, mime_type
             FROM assets WHERE item_id=?1 ORDER BY asset_id",
        )
        .map_err(db_error)?;
    let rows = statement
...
fn to_i64(value: u64) -> Result<i64, CoreError> {
    i64::try_from(value).map_err(|_| {
        CoreError::new(
            ErrorKind::Persistence,
            "numeric value exceeds SQLite range",
            false,
        )
    })
}
...
mod tests {
    ...
    fn item(completed: bool) -> LibraryItem {
        LibraryItem {
            ...
            source: SourceIdentity {
                ...
                canonical_url: Some("https://fixture.invalid/1".into()),
            },
    ...
    #[test]
    fn promotion_is_atomic_and_searchable() {
        let store = LibraryStore::open_in_memory().unwrap();
        let complete = item(true);
        store.stage_asset("job-1", &complete.assets[0]).unwrap();
        store.promote_completed("job-1", &complete).unwrap();
        assert!(store.staged_job_ids().unwrap().is_empty());
        let found = store.list(Some("Video")).unwrap();
        assert_eq!(found, vec![complete]);
    }

    #[test]
    fn incomplete_item_cannot_be_promoted() {
        let store = LibraryStore::open_in_memory().unwrap();
        let error = store.promote_completed("job-1", &item(false)).unwrap_err();
        assert_eq!(error.kind, ErrorKind::IntegrityFailure);
        assert!(store.list(None).unwrap().is_empty());
    }

    #[test]
    fn playback_rename_and_delete_round_trip() {
        let store = LibraryStore::open_in_memory().unwrap();
        store.promote_completed("job-1", &item(true)).unwrap();
        assert!(store.rename("item-1", "Renamed").unwrap());
        assert!(store.save_playback_position("item-1", 12_345).unwrap());
        let updated = store.get("item-1").unwrap().unwrap();
        assert_eq!(updated.display_title, "Renamed");
        assert_eq!(updated.playback_position_ms, 12_345);
        let deleted = store.delete("item-1").unwrap().unwrap();
        assert_eq!(deleted.item_id, "item-1");
        assert!(store.get("item-1").unwrap().is_none());
    }

    #[test]
    fn durable_download_snapshot_round_trips() {
        let store = LibraryStore::open_in_memory().unwrap();
        let snapshot = DurableDownloadSnapshot {
            job_id: "job-1".into(),
            ...
            last_error: Some(CoreError::new(ErrorKind::NetworkTimeout, "timed out", true)),
        };
