use crate::{curate_quality_choices, normalize_extracted_media, recognize_youtube_video_url, Compatibility, CoreError, DownloadPlan, DownloadPlanAsset, ErrorKind, ExtractedStream, ExtractedYouTubeMedia, MediaInfo, MediaKind, MediaSource, QualityChoice, SourceFuture, StreamRole};
...
#[derive(Debug, Clone)]
pub struct YouTubeSource { client: Client }

impl Default for YouTubeSource {
    fn default() -> Self {
        Self { client: Client::builder().connect_timeout(Duration::from_secs(10)).timeout(Duration::from_secs(20)).redirect(reqwest::redirect::Policy::limited(3)).build().expect("static YouTube HTTP client configuration") }
    }
}
...
impl YouTubeSource {
    fn fetch(&self, url: &str) -> Result<ExtractedYouTubeMedia, CoreError> {
        ...
        let response = self.client.get(&recognized.canonical_url).header("User-Agent", "Mozilla/5.0 OfflineYTPlayer/1").send().map_err(network_error)?;
        if !response.status().is_success() { return Err(CoreError::new(ErrorKind::HttpStatus, "YouTube returned a non-success HTTP status", response.status().is_server_error())); }
        if response.content_length().is_some_and(|n| n > MAX_WATCH_BYTES as u64) { return Err(source_changed("YouTube watch response exceeded the parser bound")); }
        let bytes = response.bytes().map_err(network_error)?;
        if bytes.len() > MAX_WATCH_BYTES { return Err(source_changed("YouTube watch response exceeded the parser bound")); }
        let html = std::str::from_utf8(&bytes).map_err(|_| source_changed("YouTube watch response was not UTF-8"))?;
...
impl MediaSource for YouTubeSource {
    ...
    fn download_plan<'a>(&'a self, media: &'a MediaInfo, choice_id: &'a str) -> SourceFuture<'a, DownloadPlan> {
        Box::pin(async move {
            ...
            let video=media.formats.iter().find(|f| f.format_id==id).ok_or_else(|| CoreError::new(ErrorKind::NoCompatibleFormat,"Selected YouTube format is unavailable",false))?;
            ...
            let video=fresh.formats.iter().find(|f| f.format_id==video.format_id).ok_or_else(|| source_changed("Selected YouTube format changed during planning"))?;
            let mut assets=vec![asset(video,"video",MediaKind::Video,&media.source.media_id)?];
            if video.role==StreamRole::VideoOnly {
                ...
                assets.push(asset(audio,"audio",MediaKind::Audio,&media.source.media_id)?);
            }
...
fn extract_player_json(html:&str)->Result<Value,CoreError>{
    let marker="ytInitialPlayerResponse = "; let start=html.find(marker).ok_or_else(||source_changed("YouTube player response marker was absent"))?+marker.len();
    let tail=&html[start..]; let end=tail.find(";</script>").or_else(||tail.find(";var ")).ok_or_else(||source_changed("YouTube player response terminator was absent"))?;
...
fn parse_player(v:&Value)->Result<ExtractedYouTubeMedia,CoreError>{
    let status=v.pointer("/playabilityStatus/status").and_then(Value::as_str).unwrap_or("UNKNOWN"); if status!="OK" { return Err(CoreError::new(ErrorKind::SourceChanged,"YouTube reports this media is unavailable",false)); }
    ...
    let mut streams=Vec::new(); for f in formats { if let Some(stream)=parse_stream(f)? { streams.push(stream); } }
    Ok(ExtractedYouTubeMedia{title,duration_ms,thumbnail_url,streams})
}

fn parse_stream(v:&Value)->Result<Option<ExtractedStream>,CoreError>{
    let Some(url)=v.get("url").and_then(Value::as_str) else { if v.get("signatureCipher").is_some()||v.get("cipher").is_some(){return Ok(None)} return Ok(None) };
    if url.len()>16*1024{return Err(source_changed("YouTube media URL exceeded bound"))}
    let mime=v.get("mimeType").and_then(Value::as_str).unwrap_or(""); let (mime_base,codecs)=mime.split_once(';').unwrap_or((mime,"")); let container=mime_base.split('/').nth(1).unwrap_or("bin").to_owned();
    let codec_list=codecs.split('"').nth(1).unwrap_or("").split(',').map(str::trim).collect::<Vec<_>>(); let has_video=mime_base.starts_with("video/");
...
#[cfg(test)] mod tests { use super::*; #[test] fn production_adapter_recognizes_only_supported_urls(){let s=YouTubeSource::default();assert!(s.can_handle("https://youtu.be/dQw4w9WgXcQ"));assert!(!s.can_handle("https://example.com/watch?v=dQw4w9WgXcQ"));} }
