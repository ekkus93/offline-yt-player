...
pub fn validate_http_url(input: &str) -> Result<Url, CoreError> {
    ...
    if trimmed.len() > 4_096 {
        return Err(CoreError::new(ErrorKind::InvalidInput, "URL is too long", false));
    }
    let url = Url::parse(trimmed).map_err(|_| CoreError::new(ErrorKind::InvalidInput, "URL is not valid", false))?;
    if !matches!(url.scheme(), "http" | "https") || url.host_str().is_none() {
        return Err(CoreError::new(ErrorKind::InvalidInput, "Only http(s) URLs with a host are supported", false));
    }
    if !url.username().is_empty() || url.password().is_some() {
        return Err(CoreError::new(ErrorKind::InvalidInput, "URLs containing credentials are not supported", false));
    }
    Ok(url)
}
...
pub fn sanitize_title(input: &str) -> String {
    let collapsed = input.chars().map(|c| if c.is_control() { ' ' } else { c }).collect::<String>().split_whitespace().collect::<Vec<_>>().join(" ");
    collapsed.chars().take(MAX_TITLE_CHARS).collect()
}

#[must_use]
pub fn sanitize_filename(input: &str) -> String {
    let mut value = input.chars().map(|c| match c {
        '/' | '\\' | ':' | '*' | '?' | '"' | '<' | '>' | '|' => '_',
        c if c.is_control() => '_',
        c => c,
    }).collect::<String>();
    value = value.trim_matches(|c: char| c == '.' || c.is_whitespace()).to_string();
    if value.is_empty() || value == "." || value == ".." { return "untitled".into(); }
    value.chars().take(180).collect()
}

pub fn validate_relative_library_path(path: &str) -> Result<(), CoreError> {
    let candidate = Path::new(path);
    if candidate.is_absolute() || path.contains('\0') || candidate.components().any(|component| matches!(component, Component::ParentDir | Component::RootDir | Component::Prefix(_))) {
        return Err(CoreError::new(ErrorKind::InvalidInput, "unsafe library path", false));
    }
    if path.is_empty() { return Err(CoreError::new(ErrorKind::InvalidInput, "empty library path", false)); }
    Ok(())
}
...
pub fn redact_sensitive(input: &str) -> String {
    ...
    for line in input.lines() {
        ...
        let redacted = if sensitive_names.iter().any(|name| lower.contains(name)) {
            line.split_once(':').map_or_else(|| "[REDACTED]".to_string(), |(name, _)| format!("{name}: [REDACTED]"))
...
fn redact_url_queries(line: &str) -> String {
    line.split_whitespace().map(|token| {
        ...
        Url::parse(trimmed).map_or_else(|_| token.to_string(), |mut url| {
            if url.query().is_some() { url.set_query(Some("REDACTED")); }
...
mod tests {
    ...
    #[test]
    fn filename_is_safe() {
        assert_eq!(sanitize_filename("../../my:video?.mp4"), "_.._my_video_.mp4");
    ...
    fn sensitive_values_are_redacted() {
        ...
        let url = format!("GET https://cdn.example/video?{}={}", "signature", "test-query-value");
