...
fn combined_fixture_extracts_metadata_and_combined_stream() {
    ...
    let normalized = crate::normalize_extracted_media("https://youtu.be/dQw4w9WgXcQ", &media).unwrap();
...
fn split_fixture_extracts_separate_av_and_subtitles() {
    ...
    let normalized = crate::normalize_extracted_media("https://www.youtube.com/watch?v=dQw4w9WgXcQ", &media).unwrap();
    assert!(normalized.formats.iter().any(|format| format.role == StreamRole::VideoOnly));
    assert!(normalized.formats.iter().any(|format| format.role == StreamRole::AudioOnly));
...
fn malformed_provider_url_is_rejected_during_normalization() {
    ...
    let error = crate::normalize_extracted_media("https://youtu.be/dQw4w9WgXcQ", &extracted).unwrap_err();
    assert_eq!(error.kind, ErrorKind::InvalidInput);
}

#[test]
fn oversized_caption_url_is_rejected() {
    let huge = format!("https://www.youtube.com/api/timedtext?x={}", "a".repeat(17_000));
